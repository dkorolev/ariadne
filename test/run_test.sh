...
if ! echo '{"ariadne_version":"0.0.8","stats":{"stdin_lines":1,"http_requests":3,"http_requests_by_method":{"GET":3}},"buffered_entries_count":0}' | $DIFF - <(curl -s localhost:$TEST_PORT/ariadne/impl/stats) ; then
  echo -e '\e[1;31mFAIL\e[0m'
  echo STOP >> $INPUT
  exit 1
fi
...
if ! echo '{"ariadne_version":"0.0.8","stats":{"stdin_lines":1,"http_requests":8,"http_requests_by_method":{"GET":8}},"buffered_entries_count":0}' | $DIFF - <(curl -s localhost:$TEST_PORT/ariadne/impl/stats) ; then
  echo -e '\e[1;31mFAIL\e[0m'
  echo STOP >> $INPUT
  exit 1
fi
...
if ! echo '{"ariadne_version":"0.0.8","stats":{"stdin_lines":3,"http_requests":12,"http_requests_by_method":{"GET":12}},"buffered_entries_count":0}' | $DIFF - <(curl -s localhost:$TEST_PORT/ariadne/impl/stats) ; then
  echo -e '\e[1;31mFAIL\e[0m'
  echo STOP >> $INPUT
  exit 1
fi
