...
// TODO(dkorolev): Add a /uptime endpoint.

// TOOD(dkorolev): Change the / endpoint to be more friendly. Make it possible to easily override.

// TODO(dkorolev): Add a load test and run it.
...
// Worked around by introducing the dummy 'Void' type for parameters and return values.
// Please see the test.
...
Impl.prototype.run = function(callback) {
  ...
  step(
    ...
    function stepStartExpress(error) {
      ...
      impl.express.get('/', constructObjectServer(function() {
        return {
      ...
      impl.express.get('/status', constructObjectServer(function() {
        return {
          ariadne_version: ariadne_version,
          status: impl.status,
        };
      }));
      impl.express.get('/stats', constructObjectServer(function() {
        return impl.stats;
      }));
      ...
      impl.express.get('/ariadne', constructObjectServer(methodsList));
      impl.express.get('/methods', constructObjectServer(methodsList));
      impl.express.get('/ariadne/impl/methods', constructObjectServer(methodsList));
      ...
      _.each(_.functions(impl.thrift_client), function(name) {
        if (name.substr(0, prefix_rpc.length) === prefix_rpc) {
          ...
          if (false) {
            ...
            impl.methods[method_name] = function(argument, callback) {
              assert(_.isFunction(callback));
              synchronized(impl.mutex, function(done) {
                impl.thrift_client[name].call(impl.thrift_client, argument, function() {
                  done();
