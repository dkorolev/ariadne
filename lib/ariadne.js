...
Impl.prototype.run = function(callback) {
  ...
  step(
    ...
    function stepStartExpress(error) {
      ...
      impl.express.use(express.bodyParser());
    ...
    function stepRespectStartupDelay(error) {
      if (error) throw error;
      var self = this;
    ...
    function stepRegisterThriftApiEndpoints(error) {
      ...
      function constructThriftEndpointHandler(name) {
        return function(request, response, next) {
          if (request.hasOwnProperty('query') && _.isObject(request.query) && request.query.hasOwnProperty('_') && _.isString(request.query._) && request.query._ !== '') {
            ...
            var message = new impl.service.types[input_type]();
            ...
            if (message) {
              impl.thrift_client[name](message, callback);
            } else {
              impl.thrift_client[name](callback);
            }
          } else {
            // Could not handle as Thrift endpoint.
            next();
          }
        };
      };
      ...
      _.each(_.functions(impl.thrift_client), function(name) {
        if (name.substr(0, prefix_rpc.length) === prefix_rpc) {
          impl.express.get(prefix_url + name.substr(prefix_rpc.length), constructThriftEndpointHandler(name));
        }
      });
