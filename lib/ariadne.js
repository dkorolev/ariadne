...
// TODO(dkorolev): Add watchdog for the client.
    ...
    function stepStartHealthzMonitoring(error) {
      ...
      setInterval(function() {
        ...
        if (idle_time_ms > period_ms * 3) {
          ...
          // TODO(dkorolev): I believe it is fine to send a 2nd Thrift request in parallel. Something to double-check one day.
        ...
        if (name.substr(0, prefix_rpc.length) === prefix_rpc) {
          ...
          if (false) {
            ...
            impl.methods[method_name] = function(argument, callback) {
              ...
              impl.thrift_client[name](argument, callback);
            };
          } else {
            impl.methods[method_name] = function(argument, callback) {
              assert(argument);
