...
Impl.prototype.run = function(callback) {
  ...
  function serveObject(source) {
    return function(request, response) {
      ...
      response.format({
        ...
        html: function() {
          if (result.beautifier && impl.html_beautifiers.hasOwnProperty(result.beautifier)) {
            impl.html_beautifiers[result.beautifier](result, response.send.bind(response));
          } else {
  ...
  step(
    ...
    function stepRegisterEndpoints(error) {
      ...
      impl.express.get('/', serveObject(function() {
        return {
      ...
      impl.express.get('/stats', serveObject(function() {
        return impl.stats;
      }));
      for (var i in impl.http_handlers) {
        var h = impl.http_handlers[i];
        impl.express[h.method](h.endpoint, serveObject(h.handler));
      }
      self();
    },
